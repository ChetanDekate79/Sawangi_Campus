...
import { fetchHosts, fetchMeters, fetchReport,generateReportUrl } from "./api";
...
const Report_wardha = () => {
  ...
  const onGenerateReport = () => {
    const url = generateReportUrl(selectedHost, selectedDevice, selectedDate, HostName, SelectedDeviceName2);
    setReportUrl(url);
  };
  ...
  const generateReportUrl = () => {
    const url = `https://wardha.hetadatain.com/api/jnmc_report?host=${selectedHost}&device_id=${selectedDevice}&date=${selectedDate}&hostname=${HostName}&devicename=${SelectedDeviceName2}`;
    return url;
  };
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <>
          {reportUrl && (
        <>
          <iframe src={reportUrl} style={{ width: "92vw", height: "75vh" }} title="Report" />
