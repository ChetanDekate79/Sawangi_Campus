...
import { fetchHosts, fetchMeters, chiller_csv } from "./api";
...
const Chiller = () => {
  ...
  const variable5 = deviceNames[4];
  const variable6 = deviceNames[5];

  const fetchParametersData = async () => {
    try {
      setIsLoadingHosts(true);
      const response = await fetch(`${BASE_URL}/getchillerParameters`); // Adjust the API URL if needed
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Check if both parameter and date are selected
      if (selectedColumnsY.length > 0 && selectedDate) {
        try {
          const data = await chiller_csv(selectedDate);
          setData(data);
    ...
    fetchData();
  }, [selectedColumnsY, selectedDate]);// Include selectedDevice, selectedDate, and selectedHost in dependency array
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      series4.tooltipText = "{y4}";
      series4.fontFamily = "Comic Sans MS";
      series4.fill = am4core.color("#c6fa72");
      series4.stroke = am4core.color("#c6fa72");
      series4.name = variable5;

      const series5 = chart.series.push(new am4charts.LineSeries());
      series5.strokeWidth = 2;
      series5.minBulletDistance = 10;
      series5.dataFields.categoryX = 'x';
      series5.dataFields.valueY = 'y5';
      series5.tooltipText = "{y5}";
      series5.fontFamily = "Comic Sans MS";
      series5.fill = am4core.color("#7372fa");
      series5.stroke = am4core.color("#7372fa");
      series5.name = variable6;
